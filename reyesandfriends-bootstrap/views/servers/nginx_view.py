...
class NginxView(Gtk.Box):
    def __init__(self):
        ...
        nginx_img = Gtk.Image()
        try:
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(nginx_icon_path, 48, 48, True)
            nginx_img.set_from_pixbuf(pixbuf)
        except Exception:
            nginx_img = Gtk.Image.new_from_file(nginx_icon_path)
        header_box.pack_start(nginx_img, False, False, 0)
        title = Gtk.Label()
        title.set_markup("<span size='x-large' weight='bold'>Configuraciones Nginx</span>")
    ...
    def _on_abrir_directorio_clicked(self, *_):
        ...
        os.system(f'xdg-open "{nginx_dir}"')
