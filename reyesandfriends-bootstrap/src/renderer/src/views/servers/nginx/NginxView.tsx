	...
	return (
		<div className="modal-backdrop" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.3)", zIndex: 1000 }}>
			<div className="modal" style={{ background: "#fff", maxWidth: 480, margin: "60px auto", padding: 24, position: "relative", maxHeight: "90vh", overflowY: "auto", borderRadius: 6 }}>
				...
				{error && (
					<div className="callout alert" style={{ marginTop: 16, marginBottom: 0 }}>{error}</div>
				)}
...
function ConfPreviewModal({ open, onClose, config, onSave }: {
	open: boolean;
	onClose: () => void;
	config?: NginxConfig;
		...
		<div className="modal-backdrop" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.3)", zIndex: 3000 }}>
			<div className="modal" style={{ background: "#fff", maxWidth: 700, margin: "40px auto", padding: 24, position: "relative", maxHeight: "95vh", minHeight: 600, overflowY: "auto", borderRadius: 6, display: "flex", flexDirection: "column" }}>
				...
				<div style={{ marginTop: 16, textAlign: "right" }}>
					...
					<button className="button primary" type="button" disabled={(saveOption === "custom" && !customPath) || !filename.trim()} onClick={async () => {
						if (onSave) {
							await onSave(
								filename,
								content,
...
function OverwriteModal({ open, onClose, onConfirm, filePath }: { open: boolean; onClose: () => void; onConfirm: () => void; filePath: string; }) {
	if (!open) return null;
	return (
		<div className="modal-backdrop" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.3)", zIndex: 4000 }}>
			<div className="modal" style={{ background: "#fff", maxWidth: 420, margin: "120px auto", padding: 24, position: "relative", borderRadius: 6 }}>
				...
				<p>Ya existe un archivo en:<br /><span style={{ fontFamily: "monospace", fontSize: 13 }}>{filePath}</span></p>
...
function NginxView() {
	...
	return (
		<div>
			...
			<NginxConfigModal open={modalOpen} onClose={() => { setModalOpen(false); setEditIndex(null); }} initialData={modalEditData} onSave={handleSave} />
			<ConfirmDeleteModal open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={handleDelete} />
			<ConfPreviewModal open={confModalOpen} onClose={() => setConfModalOpen(false)} config={confPreviewConfig} onSave={async (filename, content, saveDir) => {
			...
			<OverwriteModal open={overwriteModal.open} filePath={overwriteModal.filePath} onClose={() => { setOverwriteModal((prev) => { prev.resolve?.(false); return { ...prev, open: false }; }); }} onConfirm={() => { setOverwriteModal((prev) => { prev.resolve?.(true); return { ...prev, open: false }; }); }} />
			<AnimatePresence>
				{toast.visible && (
					<motion.div className={`callout ${toast.type === "error" ? "alert" : "success"}`} style={{ position: "fixed", bottom: 24, right: 24, margin: 0, borderRadius: 0, minWidth: 220, maxWidth: 400, zIndex: 1000, fontSize: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.2)", whiteSpace: "pre-line" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.25 }}>{toast.message}</motion.div>
				)}
			</AnimatePresence>
		</div>
	);
