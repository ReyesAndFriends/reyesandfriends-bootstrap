...
function MySQLView() {
    ...
    useEffect(() => {
        window.mysqlScriptsAPI?.getAll().then(setRows);
    }, []);
    ...
    const handleSave = async (data: any) => {
        let newRows: any[] = [];
        if (editIndex === null) {
            newRows = await window.mysqlScriptsAPI.add(data);
        } else {
            newRows = await window.mysqlScriptsAPI.update(editIndex, data);
        }
        setRows(newRows);
        setModalOpen(false);
        setEditIndex(null);
    ...
    const handleDelete = async () => {
        if (selectedRow !== null) {
            const newRows = await window.mysqlScriptsAPI.removeAt(selectedRow);
            setRows(newRows);
        }
        ...
        <div>
            ...
            <div className="grid-x" style={{ marginTop: 32 }}>
                <div className="cell small-12" style={{ padding: "0 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button
                        ...
                        type="button"
                        onClick={() => window.mysqlScriptsAPI?.openScriptsDir()}
                    >
            ...
            <div className="grid-x align-middle" style={{ marginTop: 16, padding: "0 32px" }}>
                ...
                <div className="cell shrink">
                    <button
                        ...
                        onClick={() => {
                            if (selectedRow !== null) {
                                setSqlPreviewData(rows[selectedRow]);
            ...
            <SQLPreviewModal
                ...
                onSave={async (filename, _content, saveDir) => {
                    ...
                    if (proceed) {
                        const res = await window.mysqlScriptsAPI.generateSQLFile(selectedRow, filename, sql, saveDir);
                        if (res.success) {
