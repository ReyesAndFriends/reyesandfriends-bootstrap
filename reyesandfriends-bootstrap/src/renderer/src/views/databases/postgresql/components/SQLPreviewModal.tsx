...
import { generatePostgreSQLScript } from "../usePostgreSQLDatabaseUtils";
...
function SQLPreviewModal({
  ...
  onClose: () => void;
  data?: any;
  onSave?: (filename: string, content: string, saveDir: string | null) => Promise<void>;
}) {
  ...
  useEffect(() => {
    if (data) {
      setFilename(`${data.dbName || "postgresql"}.sql`);
      setSql(generatePostgreSQLScript(data));
    }
  }, [data, open]);
  ...
  useEffect(() => {
    window.settingsAPI?.getWorkdir().then((dir) => {
      setWorkdir(dir);
      setDefaultSaveDir(data?.dbName ? `${dir}/databases/postgresql/${data.dbName}` : `${dir}/databases/postgresql`);
    });
  ...
  return (
    <div className="modal-backdrop" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.3)", zIndex: 1000 }}>
      <div className="modal" style={{ background: "#fff", maxWidth: 700, margin: "40px auto", padding: 24, position: "relative", maxHeight: "95vh", minHeight: 600, overflowY: "auto", borderRadius: 6, display: "flex", flexDirection: "column" }}>
        <h3 style={{ marginTop: 0 }}>Vista previa del script SQL</h3>
        ...
        <div style={{ marginTop: 16, textAlign: "right" }}>
          <button className="button secondary" type="button" onClick={onClose} style={{ marginRight: 8 }}>
            Cancelar
          </button>
          <button
            ...
            onClick={async () => {
              if (onSave) {
                await onSave(
                  filename,
                  sql,
                  saveOption === "default" ? workdir + "/databases/postgresql" : customPath || null
                );
              }
            }}
          >
